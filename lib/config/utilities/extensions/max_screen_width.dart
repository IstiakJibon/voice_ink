...
extension StringExtension on String? {
  ...
  String toYouTubeId({bool trimWhitespace = true}) {
    String url = validate();
    if (!url.contains('http') && (url.length == 11)) return url;
    if (trimWhitespace) url = url.trim();
  ...
  String convertYouTubeUrlToId({bool trimWhitespace = true}) {
    return toYouTubeId(trimWhitespace: trimWhitespace);
  }

  /// Returns YouTube thumbnail for given video id
  String getYouTubeThumbnail({bool trimWhitespace = true}) {
    return 'https://img.youtube.com/vi/${toYouTubeId(trimWhitespace: trimWhitespace)}/maxresdefault.jpg';
  }
